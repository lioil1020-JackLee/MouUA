...
# Suppress asyncua and OPC UA library warnings (but keep our own logs)
try:
...
class IoTApp(QMainWindow):
    ...
    # 定义信号以在 UI 线程启动 OPC 同步定时器
    request_opc_sync_start = pyqtSignal(int)
    # 定义信号以显示 OPC UA 错误消息
    opc_error_occurred = pyqtSignal(str, str)  # title, message
    # 定义信号以更新状态栏消息
    ...
    def __init__(self):
        ...
        self._monitor_tag_rows = {}  # 初始化 _monitor_tag_rows 避免 AttributeError
        ...
        # === 初始化 OPC 同步定时器（必须在 UI 线程中创建） ===
        self._opc_sync_timer = QTimer(self)
        self._opc_sync_timer.timeout.connect(self.sync_data_to_opcua)
        # 连接信号处理器，在 UI 线程启动定时器
        self.request_opc_sync_start.connect(self._handle_opc_timer_start)
        # 连接错误信号处理器
        self.opc_error_occurred.connect(self._handle_opc_error)
        # 连接状态栏信号处理器
        self.update_status_message.connect(self._handle_status_message)
        ...
        try:
            ...
            # 如果沒有找到 last_project.txt 中的有效路徑，則使用 temp.json
            if not loaded and self._temp_json and os.path.exists(self._temp_json):
                try:
    ...
    def open_opcua_settings(self):
        ...
        # 互鎖檢查：runtime 運行時禁止修改 OPC UA 設定
        if self._check_runtime_lock("OPC UA 設定"):
            return
        try:
            from ui.dialogs.opcua_dialog import OPCUADialog
        except Exception:
        ...
        if dlg.exec():
            try:
                ...
                # ✅ Show UI feedback: disable dialog, show status message
                try:
                    dlg.setEnabled(False)
                    if hasattr(self, "statusBar"):
                        self.statusBar().showMessage("OPC UA Server 設定中...", 0)
                except Exception:
                    pass
                ...
                # ✅ Restore UI feedback after apply
                try:
                    if hasattr(self, "statusBar"):
                        self.statusBar().showMessage("OPC UA Server 已就緒", 3000)
                except Exception:
                    pass
                ...
                try:
                    ...
                    try:
                        if getattr(self, "opc_server", None):
                            ...
                            # Ensure write callback is registered
                            if hasattr(self.opc_server, "set_write_request_callback"):
                                self.opc_server.set_write_request_callback(
                                    self._handle_opcua_write_request
                                )
                            # Reload all tags
                            QTimer.singleShot(
                                100,
    ...
    def apply_opcua_settings(self, settings):
        ...
        try:
            ...
            # Also check if application name changed (requires restart)
            old_app_name = (
                getattr(existing_server, "settings", {}).get("application_name")
                or getattr(existing_server, "settings", {}).get("application_Name")
                or getattr(existing_server, "settings", {})
                .get("general", {})
            ...
            new_app_name = (
                flat.get("application_name")
                or flat.get("application_Name")
                or nested.get("general", {}).get("application_name")
                or nested.get("general", {}).get("application_Name")
            )
            app_name_changed = (
                (old_app_name != new_app_name) if old_app_name is not None else False
            )

            # Also check if security policies changed (requires restart)
            old_security_policies = getattr(existing_server, "settings", {}).get("security_policies", {})
            new_security_policies = nested.get("security_policies", {})
            security_policies_changed = (old_security_policies != new_security_policies)

            # Also check if authentication settings changed (requires restart)
            old_authentication = getattr(existing_server, "settings", {}).get("authentication", {})
            new_authentication = nested.get("authentication", {})
            authentication_changed = (old_authentication != new_authentication)

            logger.info(
                f"OPC UA check: old_port={old_port}, new_port={new_port}, port_changed={port_changed}, "
                f"old_app={old_app_name}, new_app={new_app_name}, app_changed={app_name_changed}, "
                f"security_changed={security_policies_changed}, auth_changed={authentication_changed}, is_running={is_running}"
            )

            # Restart if port, application name, security policies, or authentication changed
            if is_running and (port_changed or app_name_changed or security_policies_changed or authentication_changed):
                logger.info(
                    f"OPC UA settings changed (port: {port_changed}, app_name: {app_name_changed}, security: {security_policies_changed}, auth: {authentication_changed}), restarting..."
                )

                # Perform restart in background to avoid blocking UI
                def restart_opc_async():
                    try:
                ...
                thread = threading.Thread(target=restart_opc_async, daemon=True)
                thread.start()
            elif is_running and not port_changed and not app_name_changed and not security_policies_changed and not authentication_changed:
                logger.info("Settings updated, reloading tags asynchronously...")
                existing_server.settings = {**flat, **nested}
                # Use reload_tags_async to avoid blocking UI (instead of reload_tags)
                QTimer.singleShot(
                    100,
                    lambda: existing_server.reload_tags_async()
                    if existing_server
                    else None,
                )
            else:
            ...
            # === 透过信号发射在 UI 线程启动同步定时器 ===
            # ✅ FIX: Use the settings we just applied, not project_config which may not exist
            try:
                # Get publish_interval from the nested settings we just processed
                general = (
                    nested.get("general", {})
                    if isinstance(nested.get("general"), dict)
                    else {}
                )

                # 获取设定的频率 (预设 1000ms)
                try:
                    interval = int(general.get("publish_interval", 1000))
                    interval = max(100, interval)  # 确保不低于 100ms 以免卡死
                except Exception:
                    interval = 1000

                # 发射信号，让 UI 线程启动定时器
                self.request_opc_sync_start.emit(interval)
                logger.info(f"OPC UA 同步定時器信號已發射: {interval}ms")
            except Exception as e:
                logger.error(f"无法启动 OPC 同步定时器: {e}")
        except Exception as e:
            logger.error(f"Error in apply_opcua_settings: {e}")
            self.opc_server = None

    @staticmethod
    def is_port_really_available(port):
        """Check if a port is really available for binding, not just unoccupied."""
        try:
            ...
            # If "Address already in use" or "Permission denied" is raised
            return False
    ...
    def _create_and_start_opc_server(self, flat: dict, nested: dict):
        ...
        try:
            ...
            # Register write callback to route OPC UA writes through ModUA.py/Monitor
            self.opc_server.set_write_request_callback(self._handle_opcua_write_request)
                ...
                try:
                    ...
                    if self.opc_server:
                        try:
                            ...
                            opc_logger.info("OPC UA tags loaded successfully")
                            # 发射信号通知 UI 线程更新状态栏
                            ...
                            if getattr(self, "_runtime_running", False):
                                # 確保 OPC server 有最新的 data_buffer 引用
                                if hasattr(self, "modbus_data_buffer") and self.modbus_data_buffer:
                                    self.opc_server.set_data_sources(
                                        data_buffer=self.modbus_data_buffer,
                                        runtime_monitor=getattr(self, "runtime_monitor", None),
                                    )
                                    opc_logger.info("✓ OPC UA data sources updated with modbus_data_buffer")
                                
                                # 獲取 OPC UA publish_interval 設定
                                opc_config = getattr(self, "project_config", {}).get("opcua_settings", {})
                                general = opc_config.get("general", {})
                                interval = int(general.get("publish_interval", 1000))
                                interval = max(100, interval)
                                
                                # 發射信號在 UI 線程啟動 timer
                                self.request_opc_sync_start.emit(interval)
                                opc_logger.info(f"✓ OPC UA 同步定時器啟動信號已發射: {interval}ms")
                                
                        except Exception as e:
                            error_msg = f"Error loading OPC tags: {str(e)}"
                            opc_logger.error(error_msg)
                            # 发射信号通知 UI 线程显示错误
                            self.opc_error_occurred.emit("標籤加載失敗", error_msg)
                except Exception as e:
                    error_msg = f"Error in OPC background initialization: {str(e)}"
                    opc_logger.error(f"{error_msg}\n{traceback.format_exc()}")
                    # 发射信号通知 UI 线程显示错误
                    self.opc_error_occurred.emit(
                        "無法啟動 OPC UA Server", f"啟動失敗：{error_msg}"
                    )
                    # Mark as not running on error
                    if self.opc_server:
                        self.opc_server.is_running = False
                    # Mark as not running on error
                    if self.opc_server:
                        self.opc_server.is_running = False
    ...
    def _handle_opc_timer_start(self, interval):
        """处理 OPC 同步定时器启动信号（在 UI 线程执行）.
        ...
        try:
            if hasattr(self, "_opc_sync_timer"):
                self._opc_sync_timer.stop()
                self._opc_sync_timer.start(interval)
                logger.info(f"OPC UA 同步定時器已在 UI 線程啟動：{interval}ms")
        except Exception as e:
            logger.error(f"启动定时器失败: {e}")

    def _handle_opc_error(self, title, message):
        """处理 OPC UA 错误消息（在 UI 线程执行）.
        ...
        try:
            ...
            logging.error(f"无法显示错误对话框: {e}")

    def _handle_status_message(self, message, timeout_ms):
        """处理状态栏消息（在 UI 线程执行）.

        Args:
            message: 要显示的消息
            timeout_ms: 消息显示时长（毫秒），0 为永久
        """
        try:
            if hasattr(self, "statusBar"):
                self.statusBar().showMessage(message, timeout_ms)
        except Exception as e:
            logging.error(f"无法更新状态栏: {e}")

    def sync_data_to_opcua(self):
        """將 ModbusDataBuffer 數據同步到 OPC UA

        這是OPC UA數據同步的核心方法，定期從ModbusDataBuffer讀取所有標籤值，
        然後通過異步調用更新OPC UA伺服器中的對應節點。

        ✅ 修復: 移除了所有重複的代碼塊，保留唯一的、完整的實現
        """
        from logging import getLogger
        ...
        # 前置檢查：伺服器和數據緩衝區必須都存在
        if not hasattr(self, 'opc_server') or not self.opc_server:
            return
        if not hasattr(self, 'modbus_data_buffer') or not self.modbus_data_buffer:
            return

        # 1. 從 ModbusDataBuffer 獲取所有點位的最新值
        try:
            all_tags = self.modbus_data_buffer.get_all_tags()
        except Exception as e:
            logger.error(f"無法取得 ModbusDataBuffer 數據: {e}")
            return

        if not all_tags:
            logger.debug("[OPC_SYNC] ModbusDataBuffer 為空，跳過同步")
            return

        # 2. 轉換為 { "路徑": 數值 } 的簡單字典
        # 重要：路徑必須與 OPC UA 節點創建時使用的路徑完全一致
        try:
            data_to_push = {
                path: tag_data.get("value") for path, tag_data in all_tags.items()
            }
        except Exception as e:
            logger.error(f"數據轉換失敗: {e}")
            return

        # 3. 檢查 OPC UA 伺服器的事件循環是否可用
        if not hasattr(self.opc_server, "loop") or not self.opc_server.loop:
            logger.debug("[OPC_SYNC] OPC 伺服器沒有事件循環，跳過同步")
            return

        if self.opc_server.loop.is_closed():
            logger.debug("[OPC_SYNC] OPC 事件循環已關閉，跳過同步")
            return

        try:
            # 4. 異步更新 OPC UA 節點值（跨線程安全）
            future = asyncio.run_coroutine_threadsafe(
                self.opc_server.update_opc_node_values(data_to_push),
                self.opc_server.loop,
            )
            # 不需要等待 future 完成 - 異步提交即可
        except Exception as e:
            logger.error(f"同步數據至 OPC UA 失敗: {e}")

    def _setup_opcua_settings_aliases(self, flat: dict, nested: dict):
        """Setup setting aliases for OPC UA."""
        try:
    ...
    # NOTE: apply_opcua_settings_impl was removed as unused dead code.
    # OPC UA settings are applied through apply_opcua_settings() method.
    ...
    def _restart_runtime_if_running(self, change_type="configuration"):
        """如果 runtime 正在運行，重新啟動以應用新的設定變更。
        
        Args:
            change_type: 變更類型描述，用於日誌
        """
        try:
            if getattr(self, "_runtime_running", False):
                logging.info(f"{change_type} changed, restarting runtime to apply new configuration")
                # 停止 runtime
                self.stop_runtime_polling()
                # 延遲重新啟動，讓停止完成
                QTimer.singleShot(
                    1000,  # 增加延遲到 1 秒
                    lambda: self.start_runtime_polling()
                )
                logging.info(f"Scheduled runtime restart in 1 second for {change_type} change")
            else:
                logging.info(f"{change_type} changed but runtime not running, no restart needed")
        except Exception as e:
            logging.warning(f"Failed to restart runtime after {change_type} change: {e}")
    ...
    def _check_runtime_lock(self, action_name="設定"):
        """檢查 runtime 是否正在運行，若是則顯示警告並返回 True（表示被鎖定）。
        
        用於互鎖功能：當 runtime 啟動時，禁止進入 Channel、Device、Group、Tag 及 OPC UA 設定頁面。
        
        Args:
            action_name: 要執行的操作名稱，用於顯示在警告訊息中。
        
        Returns:
            bool: True 表示被鎖定（runtime 正在運行），False 表示可以繼續操作。
        """
        if getattr(self, "_runtime_running", False):
            QMessageBox.warning(
                self,
                "Runtime 運行中",
                f"無法進行「{action_name}」操作。\n\n"
                f"請先停止 Runtime（點擊 🔴 Runtime 按鈕）後再進行設定變更。"
            )
            return True
        return False
    ...
    def on_edit_channel(self, item):
        ...
        if item is None:
            return
        # 互鎖檢查：runtime 運行時禁止編輯 Channel
        if self._check_runtime_lock("編輯 Channel"):
            return
        try:
        ...
        if dlg.exec():
            ...
            # 如果 runtime 正在運行，需要重新啟動以應用新的 channel 設定
            self._restart_runtime_if_running("Channel settings")
    ...
    def on_edit_device(self, item):
        ...
        if item is None:
            return
        # 互鎖檢查：runtime 運行時禁止編輯 Device
        if self._check_runtime_lock("編輯 Device"):
            return
        try:
        ...
        if dlg.exec():
            ...
            try:
                ...
                pass
            # 如果 runtime 正在運行，需要重新啟動以應用新的 device 設定
            self._restart_runtime_if_running("Device settings")
    ...
    def on_edit_tag(self, item):
        ...
        if item is None:
            return
        # 互鎖檢查：runtime 運行時禁止編輯 Tag
        if self._check_runtime_lock("編輯 Tag"):
            return
        try:
            parent = item.parent() or None
            data = {
                "general": {
                    ...
                    "description": self._safe_data(item, 1, "") or "",
                    "access": self._safe_data(item, 3, MODBUS_DEFAULT_TAG["access"]) or MODBUS_DEFAULT_TAG["access"],
                    "scan_rate": self._safe_data(item, 5, MODBUS_DEFAULT_TAG["scan_rate"]) or MODBUS_DEFAULT_TAG["scan_rate"],
                },
        ...
        if dlg.exec():
            ...
            try:
                ...
                pass
            # 如果 runtime 正在運行，需要重新啟動以應用新的 tag 設定
            self._restart_runtime_if_running("Tag settings")
            parent = parent or getattr(self.tree, "conn_node", None)
            if parent:
                self.update_right_table(parent, 0)
            # 確保在編輯 Tag 後更新 temp.json
            self._schedule_temp_export()
    ...
    def on_edit_group(self, item):
        ...
        if item is None:
            return
        # 互鎖檢查：runtime 運行時禁止編輯 Group
        if self._check_runtime_lock("編輯 Group"):
            return
        try:
            data = {
                "general": {
    ...
    def _write_monitor_value(self, pos):
        ...
        try:
            ...
            import re
            array_index = None
            tag_name_clean = tag_path
            match = re.search(r"\s*\[(\d+)\]\s*$", tag_path)
            if match:
                array_index = int(match.group(1))
                tag_name_clean = re.sub(r"\s*\[\d+\]\s*$", "", tag_path).strip()

            # print(f"[DEBUG] Parsed tag_name: {tag_name_clean}, array_index: {array_index}")

            # 簡化方式：直接構造基本的標籤信息字典
            # 先從樹結構中查找完整的標籤數據
            tag_info = self._build_tag_info_from_tree(tag_name_clean)
            if not tag_info:
            ...
            if array_index is not None:
                base_address_str = str(tag_info.get("address", "0"))
                data_type = tag_info.get("data_type", MODBUS_DEFAULT_TAG["data_type"])
                register_size = self._get_register_size_per_element(data_type)
                
                # 提取純數字address，移除array大小部分
                import re
                match = re.match(r"(\d+)", base_address_str)
                if match:
                    base_address = int(match.group(1))
                    adjusted_address = base_address + array_index * register_size
    ...
    def _build_tag_info_from_tree(self, tag_path):
        ...
        try:
            ...
            # 提取路徑各部分（Channel.Device.Group.Tag 格式）
            if GROUP_SEPARATOR in tag_path:
                parts = tag_path.split(GROUP_SEPARATOR)
                tag_name = parts[-1].strip()
                # 路徑格式: Channel.Device.[Group.]Tag
                channel_name = parts[0].strip() if len(parts) > 0 else None
            ...
            if hasattr(self, "project") and self.project and "channels" in self.project:
                # print(f"[BUILD_TAG_INFO] 方式1: 從 project 配置搜索 tag '{tag_name_clean}' (channel={channel_name}, device={device_name})")
                for channel in self.project["channels"]:
                    if "children" not in channel:
                        continue
                    # ✅ 如果指定了 channel_name，只在匹配的 channel 下搜索
                    if channel_name and channel.get("text") != channel_name:
                        continue
                    
                    for device in channel["children"]:
                        if device.get("type") != "Device":
                            continue
                        # ✅ 如果指定了 device_name，只在匹配的 device 下搜索
                        if device_name and device.get("text") != device_name:
                            continue
                        ...
                        def find_tag_recursive(item_list):
                            ...
                            for item in item_list:
                                if (
                                    ...
                                    tag_data["device_encoding"] = device.get("encoding", {})
                                    # print(
                        ...
                        if tag_data:
                            # 添加 channel 和 device 名稱以便正確選擇 worker
                            tag_data["channel_name"] = channel.get("text", "Channel1")
                            tag_data["device_name"] = device.get("text", "Device1")
                            return tag_data

            # ===== 方式2：從樹結構(UI)中直接搜索作為備選 =====
            # print(f"[BUILD_TAG_INFO] 方式1 未找到，嘗試方式2: 從 UI tree 搜索 tag '{tag_name_clean}'")
            root = self.tree.invisibleRootItem()

            def search_tree_for_tag(node, current_channel=None, current_device=None):
                """遍歷樹結構尋找標籤，追蹤當前的 channel 和 device"""
                if not node:
                    return None
                
                # 追蹤當前的 channel 和 device
                node_type = node.data(0, Qt.ItemDataRole.UserRole)
                if node_type == "Channel":
                    current_channel = node.text(0)
                elif node_type == "Device":
                    current_device = node.text(0)
                ...
                if node_type == "Tag":
                    ...
                    # ✅ 檢查 tag 名稱匹配，且 channel/device 也匹配
                    if node_text_clean == tag_name_clean:
                        # 檢查 channel 和 device 是否匹配（如果有指定的話）
                        if channel_name and current_channel != channel_name:
                            return None  # Channel 不匹配，跳過
                        if device_name and current_device != device_name:
                            return None  # Device 不匹配，跳過
                        ...
                        tag_data = {
                            ...
                            "data_type": self._safe_data(
                                node, 2, MODBUS_DEFAULT_TAG["data_type"]
                            ),  # 數據類型在角色 2
                            "read_write": self._safe_data(
                                node, 3, MODBUS_DEFAULT_TAG["access"]
                            ),  # 讀寫在角色 3
                            "scaling": self._safe_data(node, 6, {}),  # ✅ Scaling 在角色 6
                            ...
                            "channel_name": current_channel,  # ✅ 直接設置 channel_name
                            "device_name": current_device,    # ✅ 直接設置 device_name
                        }

                        # print(f"[BUILD_TAG_INFO] 方式2 找到: tag='{tag_name_clean}' channel='{current_channel}' device='{current_device}'")
                        ...
                        while parent and parent != root:
                            ...
                            if parent_type == "Device":
                                device_node = parent
                                tag_data["device_name"] = parent_text
                            elif parent_type == "Channel":
                                channel_node = parent
                                tag_data["channel_name"] = parent_text

                            parent = parent.parent()
                        
                        # 從 device_node 提取 device_id 和其他信息
                        if device_node:
                            device_text = device_node.text(0)
                            try:
                                # 從項目配置中查找此設備
                                if hasattr(self, "project") and self.project:
                                    for ch in self.project.get("channels", []):
                                        for dev in ch.get("children", []):
                                            if dev.get("text") == device_text:
                                                ...
                                                # 從設備節點獲取編碼設定
                                                device_encoding = device_node.data(5, Qt.ItemDataRole.UserRole)
                                                if isinstance(device_encoding, dict):
                                                    tag_data["device_encoding"] = device_encoding
                                                # 確保 channel_name 也被設置
                                                if not tag_data.get("channel_name"):
                                                    tag_data["channel_name"] = ch.get("text", "Channel1")
                                                break
                            except:
                                pass
                ...
                # 遞歸搜索子節點，傳遞當前的 channel 和 device
                for i in range(node.childCount()):
                    result = search_tree_for_tag(node.child(i), current_channel, current_device)
                    if result:
                        return result
            ...
            for i in range(root.childCount()):
                result = search_tree_for_tag(root.child(i), None, None)
                if result:
                    return result

            # print(f"[BUILD_TAG_INFO] 未找到 tag '{tag_name_clean}' (channel={channel_name}, device={device_name})")
            return None
    ...
    def _get_register_size_per_element(self, data_type):
        """獲取每個數據類型元素佔用的register數量"""
        if not data_type:
            return 1
        
        dt_lower = str(data_type).lower()
        
        # 從validators.py的SIZE_MAP獲取大小
        size_map = {
            "boolean": 1,
            "char": 1,
            "byte": 1,
    ...
    def _handle_opcua_write_request(self, tag_path: str, value, tag_info: dict) -> bool:
        """Handle write request from OPC UA server.

        Routes the write through the same path as Monitor table writes,
        ensuring proper Modbus encoding is applied.

        Args:
            tag_path: Full tag path (e.g., "Channel1.Device1.Data.TagName")
            value: Value to write
            tag_info: Tag information from OPC UA server (basic info)

        Returns:
            True if write was successfully queued, False otherwise
        """
        from logging import getLogger
        ...
        try:
            logger.info(f"[OPC_WRITE] Handling OPC UA write: {tag_path} = {value}")
            # print(f"[OPC_WRITE] Handling OPC UA write: {tag_path} = {value}")
            ...
            logger.debug(f"[OPC_WRITE] Parsed tag_name: {tag_name_clean}, array_index: {array_index}")

            # Build complete tag_info from tree (same as _write_monitor_value does)
            full_tag_info = self._build_tag_info_from_tree(tag_name_clean)
            if not full_tag_info:
                logger.warning(f"[OPC_WRITE] Tag not found in tree: {tag_path}")
                # print(f"[OPC_WRITE] Tag not found in tree: {tag_path}")
                return False

            logger.debug(f"[OPC_WRITE] Built tag_info: {full_tag_info}")
            # print(f"[OPC_WRITE] Built tag_info keys: {full_tag_info.keys()}")

            # 如果是陣列元素，調整地址
            if array_index is not None:
                base_address_str = str(full_tag_info.get("address", "0"))
                data_type = full_tag_info.get("data_type", MODBUS_DEFAULT_TAG["data_type"])
                register_size = self._get_register_size_per_element(data_type)
                ...
                if match:
                    base_address = int(match.group(1))
                    adjusted_address = base_address + array_index * register_size
                    full_tag_info["address"] = adjusted_address
                    logger.debug(f"[OPC_WRITE] Adjusted address for array index {array_index}: {base_address} -> {adjusted_address} (register_size={register_size})")
                else:
                    logger.warning(f"[OPC_WRITE] Could not parse base address: {base_address_str}")

            # Get address
            address = full_tag_info.get("address")
            if address is None:
                logger.warning(f"[OPC_WRITE] No address in tag_info for: {tag_path}")
                # print(f"[OPC_WRITE] No address found for: {tag_path}")
                return False

            # Select write function code using existing logic
            fc = self._select_write_function_code(full_tag_info, value)
            logger.info(f"[OPC_WRITE] Selected FC={fc} for address={address}")
            # print(f"[OPC_WRITE] Selected FC={fc} for address={address}")

            # Execute write (same path as Monitor table)
            # Note: _execute_tag_write shows message box on success/failure
            # For OPC UA writes, we call a silent version
            self._execute_tag_write_silent(address, fc, value, full_tag_info)
    ...
    def _execute_tag_write_silent(self, address, fc, value, tag_item):
        """Execute tag write without UI dialogs (for OPC UA writes).

        This is a silent version of _execute_tag_write that doesn't show
        message boxes, suitable for programmatic writes from OPC UA.
        """
        from logging import getLogger
        ...
        # print(
        #     f"[EXECUTE_WRITE_SILENT] Called with: address={address}, fc={fc}, value={value}"
        # )

        try:
            # Check Runtime status
            if not hasattr(self, "runtime_monitor") or not self.runtime_monitor:
                logger.error("[EXECUTE_WRITE_SILENT] runtime_monitor not available")
                return False

            # Check workers
            workers = getattr(self.runtime_monitor, "_workers", {})
            if not workers:
                logger.error("[EXECUTE_WRITE_SILENT] No workers available")
                return False

            # Get channel and device name for correct worker matching
            # Worker keys are in format "ChannelName_DeviceName" (e.g., "Channel1_Device1")
            channel_name = tag_item.get("channel_name")
            device_name = tag_item.get("device_name")
            device_id = tag_item.get("device_id")
            
            # 調試輸出：顯示可用的 workers 和要匹配的信息
            # print(f"[EXECUTE_WRITE_SILENT] Available workers: {list(workers.keys())}")
            # print(f"[EXECUTE_WRITE_SILENT] Looking for: channel_name={channel_name}, device_name={device_name}, device_id={device_id}")
            
            if not device_id:
                logger.warning("[EXECUTE_WRITE_SILENT] No device_id in tag_item")
                return False

            # Find worker by channel_name + device_name (primary method)
            worker = None
            config_id = None
            
            # Primary: Use channel_name and device_name to build exact config_id
            if channel_name and device_name:
                expected_config_id = f"{channel_name}_{device_name}"
                # print(f"[EXECUTE_WRITE_SILENT] Trying exact config_id: {expected_config_id}")
                if expected_config_id in workers:
                    worker = workers[expected_config_id]
                    config_id = expected_config_id
                    # print(f"[EXECUTE_WRITE_SILENT] ✓ Found worker by exact config_id: {config_id}")
                    logger.debug(f"[EXECUTE_WRITE_SILENT] Found worker by exact config_id: {config_id}")
            
            # Fallback: Try to match by device name pattern in config_id
            if not worker:
                for key, w in workers.items():
                    # config_id format: "ChannelName_DeviceName"
                    if device_name and f"_{device_name}" in str(key):
                        worker = w
                        config_id = key
                        logger.debug(f"[EXECUTE_WRITE_SILENT] Found worker by device_name match: {config_id}")
                        break
                    elif f"_Device{device_id}" in str(key) or f"Device{device_id}" in str(key):
                        worker = w
                        config_id = key
                        break

            # Last resort: Use first worker (with warning)
            if not worker and workers:
                config_id = list(workers.keys())[0]
                worker = workers[config_id]
                logger.warning(f"[EXECUTE_WRITE_SILENT] No exact match found, using first worker: {config_id}")

            if not worker:
                logger.error(
                    f"[EXECUTE_WRITE_SILENT] No worker found for device_id={device_id}"
                )
                return False
            ...
            # Read device encoding settings
            device_encoding = tag_item.get("device_encoding", {})
            if not device_encoding:
                # If no encoding, use defaults from device dialog
                from core.config.constants import MODBUS_DEFAULT_ENCODING
                device_encoding = MODBUS_DEFAULT_ENCODING.copy()

            # Map encoding
            from core.modbus.modbus_mapping import map_endian_names_to_constants, reverse_scaling
            ...
            # Reverse scaling: if tag has scaling configured, convert scaled value back to raw value
            scaling_config = tag_item.get("scaling") or tag_item.get("general", {}).get("scaling", {})
            # Add data_type to scaling config for reverse_scaling to determine if rounding is needed
            if scaling_config and isinstance(scaling_config, dict):
                scaling_config = scaling_config.copy()  # Don't modify original
                scaling_config["raw_data_type"] = tag_item.get("data_type", "")
            raw_value = reverse_scaling(value, scaling_config)
            logger.debug(f"[WRITE_SILENT] Reverse scaling: {value} -> {raw_value} (scaling={scaling_config})")

            # Build tag_info for write queue
            write_tag_info = {
                "address": address,
            ...
            # Enqueue write with reverse-scaled raw value
            success = worker._write_queue.enqueue(
                address, selected_fc, raw_value, write_tag_info
            )

            if success:
                logger.info(
                    f"[EXECUTE_WRITE_SILENT] Write enqueued: addr={address} fc={selected_fc} value={value}"
                )
                # print(
                #     f"[EXECUTE_WRITE_SILENT] Write enqueued: addr={address} fc={selected_fc} value={value}"
                # )
                ...
                logger.warning(
                    f"[EXECUTE_WRITE_SILENT] Write queue full or enqueue failed"
                )
                # print(f"[EXECUTE_WRITE_SILENT] Write queue full or enqueue failed")
    ...
    def _execute_tag_write(self, address, fc, value, tag_item):
        ...
        try:
            ...
            # 獲取 channel 和 device 名稱以正確匹配 worker
            # Worker keys are in format "ChannelName_DeviceName" (e.g., "Channel1_Device1")
            channel_name = tag_item.get("channel_name")
            device_name = tag_item.get("device_name")
            device_id = tag_item.get("device_id")
            ...
            # 從 _workers 字典中查找
            # workers 的鍵格式是 "ChannelName_DeviceName"
            worker = None
            config_id = None

            # 主要方法：使用 channel_name 和 device_name 構建精確的 config_id
            if channel_name and device_name:
                expected_config_id = f"{channel_name}_{device_name}"
                if expected_config_id in workers:
                    worker = workers[expected_config_id]
                    config_id = expected_config_id
                    # print(f"[DEBUG] Found worker by exact config_id: {config_id}")

            # 備選方法：嘗試通過設備名稱匹配
            if not worker:
                for key, w in workers.items():
                    # config_id 格式: "ChannelName_DeviceName"
                    if device_name and f"_{device_name}" in str(key):
                        worker = w
                        config_id = key
                        # print(f"[DEBUG] Found worker by device_name match: {config_id}")
                        break
                    elif f"_Device{device_id}" in str(key) or f"Device{device_id}" in str(key):
                        worker = w
                        config_id = key
                        # print(f"[DEBUG] Found worker by device_id match: {key}")
                        break

            # 最後手段：使用第一個 worker（帶警告）
            if not worker and workers:
                config_id = list(workers.keys())[0]
                worker = workers[config_id]
            ...
            device_encoding = tag_item.get("device_encoding", {})
            if not device_encoding:
                # 如果沒有編碼設定，使用預設值（來自 device dialog 的預設）
                from core.config.constants import MODBUS_DEFAULT_ENCODING
                device_encoding = MODBUS_DEFAULT_ENCODING.copy()
            ...
            # 反向縮放：如果 tag 設定了 scaling，將 scaled value 轉換回 raw value
            scaling_config = tag_item.get("scaling") or tag_item.get("general", {}).get("scaling", {})
            raw_value = reverse_scaling(value, scaling_config)
            logger.debug(f"[WRITE] Reverse scaling: {value} -> {raw_value} (scaling={scaling_config})")
            ...
            success = worker._write_queue.enqueue(address, selected_fc, raw_value, tag_info)
    ...
    def on_new_channel(self, parent_item):
        ...
        # - 展開父節點並刷新右側表格。
        # 互鎖檢查：runtime 運行時禁止新增 Channel
        if self._check_runtime_lock("新增 Channel"):
            return
    ...
    def on_new_device(self, channel_item):
        ...
        # 互鎖檢查：runtime 運行時禁止新增 Device
        if self._check_runtime_lock("新增 Device"):
            return
        ...
        if dialog.exec():
            ...
            # 確保在建立 Device 後更新 temp.json，以便啟動時能載入完整資料
            try:
                if getattr(self, "_temp_json", None):
                    QTimer.singleShot(
                        50,
    ...
    def on_new_group(self, parent_item):
        ...
        # 互鎖檢查：runtime 運行時禁止新增 Group
        if self._check_runtime_lock("新增 Group"):
            return
        group_count = sum(
            1
        ...
        if dlg.exec():
            ...
            parent_item.setExpanded(True)
            self.update_right_table(parent_item, 0)
            # 確保在建立 Group 後更新 temp.json，以便啟動時能載入完整資料
            try:
                if getattr(self, "_temp_json", None):
                    QTimer.singleShot(
                        50,
    ...
    def on_new_tag(self, parent_item):
        ...
        # 互鎖檢查：runtime 運行時禁止新增 Tag
        if self._check_runtime_lock("新增 Tag"):
            return
        try:
            try:
                ptype = (
        ...
        try:
                ...
                dlg.load_data(
                    {
                        "general": {
                            ...
                            "data_type": "Word",
                            "access": MODBUS_DEFAULT_TAG["access"],
                            "description": "",
                        },
                        "scaling": {"type": "None"},
                    }
            ...
            if dlg.exec():
                ...
                try:
                    ...
                    pass
                # 確保在建立 Tag 後更新 temp.json，以便啟動時能載入完整資料
                try:
                    if getattr(self, "_temp_json", None):
                        QTimer.singleShot(
                            50,
    ...
    def save_project_as(self):
        ...
        if not path.lower().endswith(".json"):
            path = path + ".json"
        try:
            # Process all pending Qt events to ensure UI data roles are fully updated
        ...
        try:
            ...
            if self.runtime_monitor is None:
                self.runtime_monitor = RuntimeMonitor(
                    ...
                    diagnostics_manager=self.diagnostics,  # Pass diagnostics manager for ADU logging
                )
                logging.info(f"Created RuntimeMonitor with signals instance and diagnostics")
                ...
                # ✅ 如果 OPC UA server 已經運行，啟動同步定時器
                # 這確保即使先啟動 Runtime 後才設置 OPC UA，也能同步數據
                if (
                    self.opc_server
                    and self.opc_server.is_running
                    and hasattr(self, "_opc_sync_timer")
                ):
                    try:
                        # ✅ 確保 OPC server 有最新的 data_buffer 引用
                        # 因為 modbus_data_buffer 是在 Runtime 啟動時創建的
                        if hasattr(self.opc_server, "set_data_sources"):
                            self.opc_server.set_data_sources(
                                data_buffer=self.modbus_data_buffer,
                            ...
                            logging.info(
                                "✓ OPC UA data sources updated with modbus_data_buffer"
                            )
                        # Ensure write callback is registered
                        if hasattr(self.opc_server, "set_write_request_callback"):
                            self.opc_server.set_write_request_callback(
                        ...
                        # ✅ 檢查 OPC server 是否有載入標籤，如果沒有就重新載入
                        tag_nodes_count = len(
                            getattr(self.opc_server, "_tag_nodes", {})
                        )
                        if tag_nodes_count == 0:
                            logging.warning(
                                "OPC UA _tag_nodes 為空，嘗試重新載入標籤..."
                            )
                            if hasattr(self.opc_server, "load_all_tags"):
                                self.opc_server.load_all_tags()
                                tag_nodes_count = len(
                                    getattr(self.opc_server, "_tag_nodes", {})
                                )
                                logging.info(
                                    f"OPC UA 標籤重新載入完成: {tag_nodes_count} 個標籤"
                                )

                        # 獲取 OPC UA publish_interval 設定
                        opc_config = getattr(self, "project_config", {}).get(
                            "opcua_settings", {}
                        )
                        general = opc_config.get("general", {})
                        interval = int(general.get("publish_interval", 1000))
                        interval = max(100, interval)  # 確保不低于 100ms

                        # 啟動同步定時器
                        self._opc_sync_timer.start(interval)
                        logging.info(
                            f"✓ OPC UA 同步定時器已啟動（從 Runtime 啟動）: {interval}ms"
                        )
                    except Exception as e:
                        logging.error(f"無法啟動 OPC 同步定時器: {e}")
            else:
                logging.info(
    ...
    def stop_runtime_polling(self):
        ...
        try:
            ...
            # Clear tracking
            if hasattr(self, "_monitor_tag_rows") and self._monitor_tag_rows:
    ...
    def _on_runtime_tag_updated(
        ...
        ✅ 增強的除錯：記錄 tag_name 的格式和 ModbusDataBuffer 的鍵值
        """
        try:
            # 增強除錯：記錄 tag_name 的格式（前5次更新）
            if update_count <= 5:
                import logging

                logging.info(
                    f"[TAG_NAME_FORMAT] tag_name='{tag_name}' (type={type(tag_name).__name__}, len={len(str(tag_name))})"
                )
    ...
    def _handle_opc_timer_start(self, interval_ms: int):
        """Handle starting OPC sync timer with given interval."""
        try:
            if hasattr(self, '_opc_sync_timer') and self._opc_sync_timer:
                self._opc_sync_timer.setInterval(interval_ms)
                self._opc_sync_timer.start()
                logging.info(f"OPC UA sync timer started with interval {interval_ms}ms")
        except Exception as e:
            logging.error(f"Failed to start OPC sync timer: {e}")

    def _handle_opc_error(self, title: str, message: str):
        """Handle OPC UA error by showing message box."""
        try:
            from PyQt6.QtWidgets import QMessageBox
            QMessageBox.warning(self, title, message)
        except Exception as e:
            logging.error(f"Failed to show OPC error dialog: {e}")

    def _handle_status_message(self, message: str, timeout_ms: int):
        """Handle status bar message update."""
        try:
            if hasattr(self, 'statusBar'):
                self.statusBar().showMessage(message, timeout_ms)
        except Exception as e:
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            if getattr(self, "_temp_json", None) and getattr(self, "controller", None):
                try:
                    logger.info("Saving project state to temp.json...")
                    self._call_controller("export_project_to_json", self._temp_json)
                    logger.info("Project state saved")
                except Exception as e:
                    logger.warning(f"Error saving to temp.json: {e}")
        except Exception:
            pass
...
def main():
    ...
    # log_file = os.path.join(log_dir, "modua_debug.log")

    # 設定根日誌器
    root_logger = logging.getLogger()
    root_logger.setLevel(logging.DEBUG)

    # 文件處理器 - 輸出所有 DEBUG 級別以上的日誌 (已移除)
