...
def _is_light_theme():
    ...
    if app:
        ...
        return window_color.lightness() > 128  # ‰∫ÆÂ∫¶Â§ßÊñº128Ë¶ñÁÇ∫Ê∑∫Ëâ≤
    return False
...
"""
    ...
    def hide_all_tags(self):
        ...
        # 2. ?ü¢ Âª∫Á?Á¨¶Â? Project -> Connectivity ?ÑÁ?ÊßãÔ??•Â??™Âª∫Á´ãÔ?
        try:
            ...
            pass
        # Á¢∫‰??ÇÂ±§ÁØÄÈªû‰Ωø?®Ë??∂‰?ÁØÄÈªûÁõ∏?åÁ?Â≠óÂ?ÔºåÈÅø?çÂ?Â≠óÂ??ñÊ®£ÂºèÈÄ†Ê?È´òÂ∫¶Â∑ÆÁï∞
        try:
    ...
    def contextMenuEvent(self, event):
        ...
        # ?πÊ?ÁØÄÈªûÈ??ãÈ°ØÁ§∫‰??åÈÅ∏??
        if node_type == "Connectivity":
            menu.addAction(
                "???∞Â? Channel", lambda: self.request_new_channel.emit(item)
            )
        elif node_type == "Channel":
            menu.addAction("???∞Â? Device", lambda: self.request_new_device.emit(item))
            menu.addSeparator()
            self._add_common_actions(menu, item)
        elif node_type in ["Device", "Group"]:
            menu.addAction("???∞Â? Group", lambda: self.request_new_group.emit(item))
            menu.addAction("???∞Â? Tag", lambda: self.request_new_tag.emit(item))
            menu.addSeparator()
            self._add_common_actions(menu, item)
            ...
            if node_type == "Device":
                menu.addSeparator()
                menu.addAction(
                    "?ì• ?ØÂÖ• CSV", lambda: self.request_import_csv.emit(item)
                )
                menu.addAction(
                    "?ì§ ?ØÂá∫ CSV", lambda: self.request_export_csv.emit(item)
                )
        elif node_type == "Tag":
            self._add_common_actions(menu, item)
    ...
    def _add_common_actions(self, menu, item):
        # ?öÁî®?∏ÂñÆ?ï‰?ÔºöÂâ™?á„ÄÅË?Ë£Ω„ÄÅË≤º‰∏ä„ÄÅÂà™?§„ÄÅÂÖßÂÆ?
        menu.addAction("?ÇÔ? ?™‰?", lambda: self.request_cut_item.emit(item))
        menu.addAction("?? Ë§áË£Ω", lambda: self.request_copy_item.emit(item))
        menu.addAction("?ì• Ë≤º‰?", lambda: self.request_paste_item.emit(item))
        menu.addSeparator()
        menu.addAction("???™Èô§", lambda: self.request_delete_item.emit(item))
        menu.addAction("?èÔ? ?ßÂÆπ", lambda: self.request_edit_item.emit(item))
